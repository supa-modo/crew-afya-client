...
const CoverageUtilization = ({
  ...
  coverageData: parentData,
  skipFetch = false,
}) => {
  const { user } = useAuth();
  const [loading, setLoading] = useState(
    parentLoading !== undefined ? parentLoading : true
  );
  const [error, setError] = useState(null);
  const [coverageData, setCoverageData] = useState(parentData || null);

  useEffect(() => {
    // If parent provides data and wants to skip fetch, use the parent data
    if (skipFetch && parentData) {
      setCoverageData(parentData);
      setLoading(false);
      return;
    }

    // If parent provides loading state, use it
    if (parentLoading !== undefined) {
      setLoading(parentLoading);
    }

    // Only fetch data if we're not skipping fetch
    if (!skipFetch) {
      fetchCoverageData();
    }
  }, [user, skipFetch, parentData, parentLoading]);

  const fetchCoverageData = async () => {
    if (!user || !user.id || skipFetch) return;
            ...
            <div className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-blue-50 to-blue-50/50 dark:from-gray-800 dark:to-gray-700 border border-blue-100 dark:border-gray-700">
              <div className="flex justify-between mb-3">
                ...
                <div className="flex items-center mt-1">
                  <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700">
                    0%
                  </span>
